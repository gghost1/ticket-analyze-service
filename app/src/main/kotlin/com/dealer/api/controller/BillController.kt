...
class BillController(private val billService: BillService) {
    ...
    fun createBill(@Valid @RequestBody request: CreateBillRequest): BillDto =
        billService.createBill(SecurityUtils.getCurrentUserId(), request)

    @GetMapping("/{id}")
    fun getBill(@PathVariable id: UUID): BillDto =
        billService.getBill(id, SecurityUtils.getCurrentUserId())

    @PatchMapping("/{id}")
    fun updateBill(@PathVariable id: UUID, @Valid @RequestBody request: UpdateBillRequest): BillDto =
        billService.updateBill(id, SecurityUtils.getCurrentUserId(), request)
    ...
    fun settleBill(@PathVariable id: UUID): BillDto =
        billService.settleBill(id, SecurityUtils.getCurrentUserId())

    @PostMapping("/{id}/items")
    @ResponseStatus(HttpStatus.CREATED)
    fun addItem(@PathVariable id: UUID, @Valid @RequestBody request: AddBillItemRequest): BillItemDto =
        billService.addItem(id, SecurityUtils.getCurrentUserId(), request)
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    fun deleteItem(@PathVariable id: UUID, @PathVariable itemId: UUID) =
        billService.deleteItem(id, itemId, SecurityUtils.getCurrentUserId())

    @PostMapping("/{id}/splits")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    fun setSplits(@PathVariable id: UUID, @Valid @RequestBody request: BillSplitsRequest) =
